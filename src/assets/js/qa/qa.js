		...
		$pagination.pagination({
			...
			callback: function(data, pagination) {
				...
				if (data.length === 0) {
					...
					$.each(data, function(index, item) {
						var html = '<div class="list-group-item list-group-item-action flex-column align-items-start question-answer" data-answer-id="' + item.id + '"><small class="text-muted"><a class="question-answer-provider" href="mailto:' + item.provider_email + '">' + item.provider_name + '</a> - <span class="question-answer-provider-major">' + item.major + '</span>&ensp;&ensp;<span class="question-answer-creation-time">' + item.time + '</span>' + (item.authentication === '1' ? '&ensp;&ensp;<span class="authenticated answer-authenticated text-sucess">' + SCLang.authenticated + '</span>' : '') + '</small>';
