...
class temp{
	...
	public function compile_to_graph($command_arr){
		...
		foreach ($command_arr as $command){
			...
			// NAME
			if(strcmp(substr($command, 0, 4), 'NAME')){
			...
			// EN_NAME
			if(strcmp(substr($command, 0, 7), 'EN_NAME')){
				$eq_pos = strpos($command, '=');
				$command = substr($command, $eq_pos + 1);
				$command = trim($command);
				$command = trim($command, '"');
			...
			// GROUP
			if(strcmp(substr($command, 0, 5), 'GROUP')){
				$eq_pos = strpos($command, '=');
				$command = substr($command, $eq_pos + 1);
				$command = trim($command);
				$command = trim($command, '"');
			...
			// VERSION
			if(strcmp(substr($command, 0, 7), 'VERSION')){
				$eq_pos = strpos($command, '=');
				$command = substr($command, $eq_pos + 1);
				$command = trim($command);
				$command = trim($command, '"');
			...
			// PROGRAM_LENGTH
			if(strcmp(substr($command, 0, 14), 'PROGRAM_LENGTH')){
				$eq_pos = strpos($command, '=');
				$command = substr($command, $eq_pos + 1);
				$command = trim($command);
				$command = trim($command, '"');
			...
			// INTRO
			if(strcmp(substr($command, 0, 5), 'INTRO')){
				$eq_pos = strpos($command, '=');
				$command = substr($command, $eq_pos + 1);
				$command = trim($command);
				$command = trim($command, '"');
			...
			// EN_INTRO
			if(strcmp(substr($command, 0, 8), 'EN_INTRO')){
				$eq_pos = strpos($command, '=');
				$command = substr($command, $eq_pos + 1);
				$command = trim($command);
				$command = trim($command, '"');
			...
			// OBJECTIVES
			if(strcmp(substr($command, 0, 10), 'OBJECTIVES')){
				$eq_pos = strpos($command, '=');
				$command = substr($command, $eq_pos + 1);
				$command = trim($command);
				$command = trim($command, '"');
			...
			// EN_OBJECTIVES
			if(strcmp(substr($command, 0, 13), 'EN_OBJECTIVES')){
				$eq_pos = strpos($command, '=');
				$command = substr($command, $eq_pos + 1);
				$command = trim($command);
				$command = trim($command, '"');
			...
			// DEGREE
			if(strcmp(substr($command, 0, 6), 'DEGREE')){
				$eq_pos = strpos($command, '=');
				$command = substr($command, $eq_pos + 1);
				$command = trim($command);
				$command = trim($command, '"');
			...
			// EN_DEGREE
			if(strcmp(substr($command, 0, 9), 'EN_DEGREE')){
				$eq_pos = strpos($command, '=');
				$command = substr($command, $eq_pos + 1);
				$command = trim($command);
				$command = trim($command, '"');
			...
			// Event
			if(strcmp(substr($command, 0, 5), 'Event')){
				...
				$node_value = substr($command, 0, $eq_pos + 1);
				$node_value = trim($node_value);

				if(strcmp($node_name, 'GRADUATION')){
					$node_type = 2;
				}else {
					$node_type = 0;
				}
		...
		for ($i = 0; $i < sizeof($temp_list); $i ++){
				...
				if (strpos($value, '&&') != FALSE){
					...
					for ($j = 0; $j < sizeof($value); $j ++){
						...
						for (; $k < sizeof($temp_list); $k ++){
							if(strcmp($node_list[$k]['node_name'], $item)){
								break;
							}
						}
						$son_list[] = array(
			...
			// CourseEvent
			if (strcmp($event_type, 'CourseEvent')){
					...
					for ($j = 1; $j < sizeof($sp_temp); $j ++){
						if ($sp_temp[$j] == '('){
							...
							if(array_key_exists($little_name, $course_status)){
								$k = 0;
								for (; $k < sizeof($node_list); $k ++){
									if(strcmp($node_list[$k]['node_name'], $little_name)){
										break;
									}
								}
								$son_list[] = array(
			...
			// ScoreEvent
			if(strcmp($event_type, 'ScoreEvent')){
				$node_id = $node_id_counter;
				$node_id_counter ++;
			...
			// VariableEvent
			if(strcmp($event_type, 'VariableEvent')){
				$value = trim($value, '()');
				$value = explode(',', $value);
				...
				for (; $j < sizeof($temp_list); $j ++){
					if(strcmp($node_list[$j]['node_name'], $value[1])){
						break;
					}
				}
				$son_list[1] = array(
